...
<div class="container mx-auto px-4 py-8">
    <h1 class="text-4xl font-bold text-white mb-4">Edit Food Type</h1>

    <div class="bg-white p-6 rounded-lg shadow-md">
        <form action="{{ route('foodtypes.update', $foodType->id) }}" method="POST">
            @csrf
            ...
            <div class="mb-4">
                ...
                <input type="text" id="name" name="name" 
                       ...
                       placeholder="{{$foodType->name}}" required>
                @error('name')
            ...
            <div class="mb-4">
                ...
                <textarea id="description" name="description" rows="4"
                          class="w-full p-2 border border-gray-300 rounded-lg focus:ring-1 focus:ring-amber-500 focus:border-amber-500"
                          placeholder="{{$foodType->description}}"></textarea>
                @error('description')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
